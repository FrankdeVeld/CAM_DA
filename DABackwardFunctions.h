...
AlgebraicVector<double> InitialXs(int Scenario,double MuEarth) {
    ...
    switch(Scenario) {
        case 1:
            ...
            xs0[0] = -20035.49963;     // sec km altitude
            xs0[1] = 0.0;          
            xs0[2] = -2.72058944;    
            xs0[3] = 0.0008565374959; 
            xs0[4] = 0.0;             
            xs0[5] = -3.153940884;      
        ...
        case 3:
            ...
            // Secondary initial position, guaranteeing a collision at tca
            xs0[0] = sec;                              // prim km altitude
            xs0[1] = 0.0;                            
            xs0[2] = 0.0;                             
            xs0[3] = 0.0;                            
            xs0[4] = sqrt(MuEarth/abs(sec))*sqrt(1-eccs);  // at apogee
            xs0[5] = 0.0;
            break;      
            }
    }
...
AlgebraicVector<double> InitialXp(int Scenario,double MuEarth) {
    ...
    switch(Scenario) {
        case 1:
            ...
            // Some random primary
            xp0[0] = prim;                              // prim km altitude
            xp0[1] = 0.0;                            
            xp0[2] = 0.0;                             
            xp0[3] = 0.0;                            
            xp0[4] = -sqrt(MuEarth/abs(prim))*sqrt(1-eccp);  // at apogee
            xp0[5] = 0.0;
            break;   
            }
        case 2:
        ...
        case 3:
            {
            // Set initial conditions
            xp0[0] = 20035.49963;     // prim km altitude
            xp0[1] = 0.0;          
            xp0[2] = 2.72058944;    
            xp0[3] = -0.0008565374959; 
            xp0[4] = 0.0;             
            xp0[5] = 3.153940884;  
            break;   
            }                     
...
DA tcaInversion(int ControlCases, AlgebraicVector<DA> xptN9DA,AlgebraicVector<double> xstN9Vec, DA DeltatCAtN9, double tcaNom, double MuEarth, double Lsc){ 
    switch(ControlCases){
        case 1:
            AlgebraicVector<DA>     xpfNew(6); 
            AlgebraicVector<double> xsfNew(6);
            xpfNew = RK78(6, xptN9DA,  {DA(7),DA(8),DA(9)}, 0.0, tcaNom*0.1,TBAcc,MuEarth,Lsc); // Propagation from t(N=9) to t(N=10) of DA object
            xsfNew = RK78(6, xstN9Vec, {0.0, 0.0, 0.0},     0.0, tcaNom*0.1,TBAcc,MuEarth,Lsc); // Propagation from t(N=9) to t(N=10) of vector object
            // Variable tCA at t(N=9) due to control between t(N=9) and t(N=10)
            DeltatCAtN9 = 0.0 + DA(10);
            AlgebraicVector<DA> xpDATCA(6), xpDATCAProp(6), xsDATCA(6), xsDATCAProp(6), xrelDATCAProp(6);

            for (int i = 0; i < 6; i ++) {
                xpDATCA[i] = xpfNew[i]; // Includes 9 DA variables
                xsDATCA[i] = xsfNew[i] + DA(i+1)*0.0; 
            }
            xpDATCAProp = KeplerProp(xpDATCA, DeltatCAtN9, MuEarth);
            xsDATCAProp = KeplerProp(xsDATCA, DeltatCAtN9, MuEarth);
            xrelDATCAProp    = xsDATCAProp - xpDATCAProp;
            DA DeltatCAtN9   = findTCA(xrelDATCAProp, 10);
            break;
        //case 2: 
